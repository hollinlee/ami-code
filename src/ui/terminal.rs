...
pub fn render_terminal_pane(
    ...
    frame.render_widget(
        Paragraph::new(styled_screen_lines(screen)).block(block),
        area,
    );
}

fn styled_screen_lines(screen: &vt100::Screen) -> Vec<Line<'static>> {
...
mod tests {
    ...
    fn preserves_cell_colors_and_inverse() {
        ...
        let lines = styled_screen_lines(parser.screen());
